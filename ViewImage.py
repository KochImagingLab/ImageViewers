...
def ortChange(value):
    ...
    if (viewer1._pixmapHandle is not None): 
        ...
        if(crosshairsBox1.checkState() == 2):
            viewer1.updateVerticalProfile(int(0))
        horscrollbar.setMaximum(int(viewer1.getImgHeight()))
        horTextbox.setText(str(0))
        horscrollbar.setValue(int(0)) 
        if(crosshairsBox1.checkState() == 2):
            viewer1.updateHorizontalProfile(int(0))
        
#         if(thruplaneBox.checkState() == 2):
#             viewer1.updateThroughPlane()
...
if __name__ == '__main__':
    ...
    if(args.niiFile != ''):
        viewer1.loadNIFTI(args.niiFile)
